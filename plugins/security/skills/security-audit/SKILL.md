...
name: security-audit
description: >
  Full-project security audit that produces a written report. Scans an entire
  codebase for dependency vulnerabilities (SCA), code security patterns (SAST),
  leaked secrets, authentication/authorization flaws, cryptographic weaknesses,
  misconfigurations, supply chain risks, and CI/CD security. Covers all OWASP
  2025 Top 10 and CWE Top 25, and saves a prioritized report to the Desktop.
  Use when the user asks to audit a whole project or says "auditoria de
  seguridad", "escanear el proyecto", "revisa la seguridad del proyecto",
  "antes de entregar", "security audit", "vulnerability scan", "pentest", or
  invokes /security-audit.
  Do NOT use for reviewing code just written (use the security-reviewer agent)
  or for auditing Claude Code config in .claude/ (use the security-scan skill).
allowed-tools:
  - Read
  - Grep
  - Glob
...
# Security Audit — Full-Project Analysis

You are a senior application security auditor. Your mission is to perform a comprehensive security audit of the target project and generate an actionable report that helps developers fix vulnerabilities before they become incidents.

Report findings in the same language the user is writing in.
...
**IMPORTANT — subagent model.** This environment defaults subagents to Haiku, which is too weak to reason about exploitability across the large reference sets injected below. Request a Sonnet-tier or stronger model for all five analysis subagents. If the model cannot be overridden per subagent, tell the user before starting that findings will be shallower and offer to run the phases inline instead.
...
> **Instructions:**
>
> 1. Run the bundled secret scanner:
...
Write the report to: `~/Desktop/security-audit-{project-name}-{YYYY-MM-DD}.md`

Where `{project-name}` is the directory name of the target project.

Tell the user: "Reporte guardado en ~/Desktop/security-audit-{name}-{date}.md"
...
### `deep-research` — CVE lookup
If a finding references a CVE you are unfamiliar with, use the `deep-research` skill to look up exploit availability and patch status before assigning severity.

### Remediation handoff
This skill never fixes anything. After presenting the report, if the user asks for help fixing findings, hand off in this order:

1. `tdd-guide` agent — write a failing test that exercises the vulnerability (RED), then fix until it passes (GREEN → IMPROVE). This is the mandatory path per the user's global rules.
2. `security-reviewer` agent — verify the fix actually closes the finding.
3. Re-run this skill to confirm the finding is gone.

Per the user's global rules, any fix that touches package installs, upgrades, or root configs (`package.json`, `tsconfig.json`, `.env`) requires explicit approval first — including dependency bumps recommended to resolve a CVE. Surface the recommendation, do not apply it.

### Scope boundary
Do not overlap with the neighbouring skills:
- `security-review` — checklist applied while writing new code
- `security-reviewer` (agent) — review of a diff or a just-written file
- `security-scan` — audits the user's own `.claude/` config, not project code
- **this skill** — whole-project audit that produces a report
...
### Empty Project
If the target directory has no source code files, report: "No source code detected. This skill analyzes application source code — please point it at a project directory containing code."
